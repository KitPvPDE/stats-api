...
public class IntVoidKeyBuilder extends IntKeyBuilder<Void> implements StatsKeyBuilder<Void, Integer> {
    ...
    public @NotNull IntSStatsKey build() {
        return new IntVoidStatsKeyImpl(this.keyBuilder.build(), this.function, this.neutral, this.def, this.offset);
    }

    @Override
    public @NotNull IntSSeasonKey season() {
        return new IntVoidSeasonKeyImpl(this.keyBuilder.build(), this.function, this.neutral, this.def, this.offset);
    }

    @Override
    public @NotNull IntSStageKey stage() {
        return new IntVoidStageKeyImpl(this.keyBuilder.build(), this.function, this.neutral, this.def, this.offset);
    }
}
